...
endmodule // efx_jtag_spi_flash_loader
 ...
 //     end
   ...
   always @(posedge clkin)
      if (~nrst) begin
         ...
         case (spi_cmd_instr[7:0])
            WRITE4BDATA  : fourbyte_cmd <= 1'b1;
            BREAD4B_DATA : fourbyte_cmd <= 1'b1;
            CHIP_ERASE_0 : cnt_max[5:0] <= 6'd8;
            CHIP_ERASE_1 : cnt_max[5:0] <= 6'd8;
            SECTOR_ERASE4B     :  begin
                                      fourbyte_cmd <= 1'b1;
                                      cnt_max[5:0] <= 6'd40;
                                  end
            BLOCK_64KB_ERASE4B :  begin
                                      fourbyte_cmd <= 1'b1;
                                      cnt_max[5:0] <= 6'd40;
                                  end
            default            :  begin 
		...
		if (spi_cmd_en_pos)
		  begin 
		     case (spi_cmd)
			  ...
			  cmd_in <= fourbyte_cmd ? BREAD4B_DATA : BREAD_DATA; //Burst Read Data
			  cmd_in_reg <= fourbyte_cmd ? BREAD4B_DATA : BREAD_DATA; //Burst Read Data
			  burst_rd_flag <= 1'b1;
		       end
			  ...
			  busy_reg <= 1'b1;
			  cmd_in <= fourbyte_cmd ? WRITE4BDATA : WRITEDATA;
			  cmd_in_reg <= fourbyte_cmd ? WRITE4BDATA : WRITEDATA;
